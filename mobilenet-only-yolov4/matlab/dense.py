...
class Densenet(nn.Module):
    ...
    def forward(self, x):
        ...
        for block in self.model.features:
            if str(type(block)) == "<class 'densenet._Transition'>":
                for _, subblock in enumerate(block):
                    x = subblock(x)
                    if str(type(subblock)) == "<class 'torch.nn.modules.conv.Conv2d'>":
...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes=1, backbone="densenet121", pretrained=False):
