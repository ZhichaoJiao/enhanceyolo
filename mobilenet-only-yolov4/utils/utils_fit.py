...
def fit_one_epoch(model_train, model, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        ...
        images, day_images, targets = batch[0], batch[1], batch[2]
        with torch.no_grad():
            if cuda:
                images  = images.cuda(local_rank)
                day_images = day_images.cuda(local_rank)
    ...
    if local_rank == 0:
        ...
        # fakeimg = fake_imgs[0] * 255
        # fakeimg = fakeimg.cpu().detach().numpy()
        # fakeimg = np.transpose(fakeimg, (1, 2, 0))
        # cv2.imwrite('F:/studyp/mobilenet-yolov4-pytorch-main/pic/' + str(epoch) + '.jpg',
        #             cv2.cvtColor(fakeimg, cv2.COLOR_RGB2BGR))
