...
class BUSD(nn.Module):
    def __init__(self):
        ...
        self.layers.append(UpsamplerBlock(ninput=160, noutput=48,
                                          up_height=int(img_height)//16, up_width=int(img_width)//16))
        self.layers.append(UpsamplerBlock(ninput=160, noutput=32,
                                          up_height=int(img_height)//8, up_width=int(img_width)//8))
        self.layers.append(UpsamplerBlock(ninput=72, noutput=24,
                                          up_height=int(img_height)//4, up_width=int(img_width)//4))
        self.layers.append(UpsamplerBlock(ninput=48, noutput=16,
                                          up_height=int(img_height)//2, up_width=int(img_width)//2))
        self.layers.append(UpsamplerBlock(ninput=32, noutput=8,
                                          up_height=int(img_height)//1, up_width=int(img_width)//1))

        self.output_conv = conv1x1(8, num_classes)

    def forward(self, x0, x1, x2, x3, x4):
        d5 = x4
        output = self.layers[0](d5)
        d4 = torch.cat((output, x3), dim=1)
        output = self.layers[1](d4)
        d3 = torch.cat((output, x2), dim=1)
        output = self.layers[2](d3)
        d2 = torch.cat((output, x1), dim=1)
        output = self.layers[3](d2)
        d1 = torch.cat((output, x0), dim=1)
        output = self.layers[4](d1)
        # for layer in self.layers:
        #     output = layer(output)

        output = self.output_conv(output)

        return output, d3, d4, d5
