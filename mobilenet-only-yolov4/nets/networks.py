...
class Unet_resize_conv(nn.Module):
    def __init__(self, opt, skip):
        ...
        self.opt = opt
        self.skip = skip
        self.attention_block = cbam_block(512)
        p = 1
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, input, gray):
        ...
        if self.opt.use_norm == 1:
            ...
            #CBAM attention
            conv5 = self.attention_block(conv5)
            ...
            if self.opt.linear:
                output = output/torch.max(torch.abs(output))
        output = self.sigmoid(output*5)*5
        #output = -2*torch.log2(1-output)
        output = torch.pow(y, output)
...
class PerceptualLoss(nn.Module):
    ...
    def compute_vgg_loss(self, vgg, img, target):
        ...
        img_fea = vgg(img_vgg).cuda()
        target_fea = vgg(target_vgg).cuda()
